...
async function getPath (jobId) {
	...
	if (rows.length < 1) {
		...
		return path.join(__dirname, "../../structure", rows[0].customer, rows[0].project, rows[0].batch,
			rows[0].source + rows[0].target, "JOBS", rows[0].job + ".uet");
	}
}

function getUrl(NODE_ENV) {
	if (NODE_ENV == "eu") {
		return "https://uet.datamundi.eu/login";
	}
	else if (NODE_ENV == "production") {
		return "https://uet.datamundi.space/login";
	}
	else {
		return "http://localhost:6634/login";
	}
}
