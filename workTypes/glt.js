...
async function getProgress(jobId) {
    ...
    const sqlGetProgress = db.prepare("SELECT COUNT(*) AS done, (SELECT COUNT(*) FROM JOB WHERE showThisRow = 1) AS total FROM JOB WHERE stamp IS NOT NULL AND showThisRow = 1");
...
async function getPath (jobId) {
    ...
    if (rows.length < 1) {
        ...
        return path.join(__dirname, "../../structure", rows[0].customer, rows[0].project, rows[0].batch,
            rows[0].source + rows[0].target, "JOBS", rows[0].job + ".glt");
    }
}

function getUrl(NODE_ENV) {
    if (NODE_ENV == "eu") {
        return "https://glt.datamundi.eu/login";
    }
    else if (NODE_ENV == "production") {
        return "https://glt.datamundi.space/login";
    }
    else {
        return "http://localhost:3999/login";
    }
}
