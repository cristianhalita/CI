...
async function getPath (jobId) {
    ...
    if (rows.length < 1) {
        ...
        return path.join(__dirname, "../../structure", rows[0].customer, rows[0].project, rows[0].batch,
            rows[0].source + rows[0].target, "JOBS", rows[0].job + ".flt");
    }
}

function getUrl(NODE_ENV) {
    if (NODE_ENV == "eu") {
        return "https://flt.datamundi.eu/login";
    }
    else if (NODE_ENV == "production") {
        return "https://flt.datamundi.space/login";
    }
    else {
        return "http://localhost:3979/login";
    }
}
