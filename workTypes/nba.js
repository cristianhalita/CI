...
async function getProgress(jobId) {
    ...
    const sqlGetProgress = db.prepare("SELECT COUNT(*) AS done, (SELECT COUNT(*) FROM domains) AS total FROM domains WHERE stamp IS NOT NULL");
...
async function getPath (jobId) {
    ...
    if (rows.length < 1) {
        ...
        return path.join(__dirname, "../../structure", rows[0].customer, rows[0].project, rows[0].batch,
            rows[0].source + rows[0].target, "JOBS", rows[0].job + ".nba");
    }
}

function getUrl(NODE_ENV) {
    if (NODE_ENV == "eu") {
        return "https://nba.datamundi.eu/api/login";
    }
    else if (NODE_ENV == "production") {
        return "https://nba.datamundi.space/api/login";
    }
    else {
        return "http://localhost:4315/login";
    }
}
