...
async function getProgress(jobId) {
    ...
    const sqlGetProgress = db.prepare("SELECT COUNT(*) AS done, (SELECT COUNT(*) FROM SENTENCES) AS total FROM SENTENCES WHERE stamp IS NOT NULL");
...
async function getPath (jobId) {
    ...
    if (rows.length < 1) {
        ...
        return path.join(__dirname, "../../structure", rows[0].customer, rows[0].project, rows[0].batch,
            rows[0].source + rows[0].target, "JOBS", rows[0].job + ".fqt");
    }
}

function getUrl(NODE_ENV) {
    if (NODE_ENV == "eu") {
        return "https://fqt.datamundi.eu/login";
    }
    else if (NODE_ENV == "production") {
        return "https://fqt.datamundi.space/login";
    }
    else {
        return "http://localhost:4362/login";
    }
}
