...
async function getPath (jobId) {
    ...
    if (rows.length < 1) {
        ...
        return path.join(__dirname, "../../structure", rows[0].customer, rows[0].project, rows[0].batch,
            rows[0].source + rows[0].target, "JOBS", rows[0].job + ".ale");
    }
}

function getUrl(NODE_ENV) {
    if (NODE_ENV == "eu") {
        return "https://ale.datamundi.eu/login";
    }
    else if (NODE_ENV == "production") {
        return "https://ale.datamundi.space/login";
    }
    else {
        return "http://localhost:8346/login";
    }
}
...
async function getPreview (jobId) {
    ...
    const sqlGetRows = db.prepare("SELECT id, source, translated_text1, translated_text2, translated_text3, translated_text4" +
        ", dnt FROM JOB LIMIT 20");
