...
async function getPath (jobId) {
    ...
    if (rows.length < 1) {
        ...
        return path.join(__dirname, "../../structure", rows[0].customer, rows[0].project, rows[0].batch,
            rows[0].source + rows[0].target, "JOBS", rows[0].job + ".bst");
    }
}

function getUrl(NODE_ENV) {
    if (NODE_ENV == "eu") {
        return "https://bst.datamundi.eu/login";
    }
    else if (NODE_ENV == "production") {
        return "https://bst.datamundi.space/login";
    }
    else {
        return "http://localhost:5130/login";
    }
}
...
async function getPreview(jobId) {
    ...
    const sqlGetRows = db.prepare("SELECT id, source, target FROM segments LIMIT 20");
